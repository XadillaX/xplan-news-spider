...
// spider pioneer
var pioneer = new SpiderPioneer("mongodb://127.0.0.1/zju-cst-xplan");
...
pioneer.setParseListFunc(function(status, html, respHeader) {
    ...
    var regExp3 = /<span class="fr">(\S+)<\/span><\/li>/g;
    var regExp4 = /<span class="fr">(\S+)<\/span><\/li>/;

    var result1 = html.match(regExp1);
    var result2 = html.match(regExp3);
    if(result1) {
        var rtn = [];
        for(var i = 0; i < result1.length; i++) {
            var newsurl ="http://www.cst.zju.edu.cn/" + result1[i].match(regExp2)[1];
            var time = parseInt(Date.create(result2[i].match(regExp4)[1]) / 1000);
            rtn.push(newsurl);
            rtn.push(time);
        }
